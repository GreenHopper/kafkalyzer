...
pub fn fetch_subjects(profile: ClusterProfile) -> Result<Vec<String>> {
    let url = profile.schema_registry_url.clone().ok_or_else(|| anyhow::anyhow!("No Schema Registry URL provided"))?;
    ...
    let subjects = rt.block_on(async {
        ...
        let resp = client.get(format!("{}/subjects", url))
            .send()
            .await?
            .json::<Vec<String>>()
            .await?;
...
pub fn fetch_schema(profile: ClusterProfile, subject: String) -> Result<String> {
    ...
    let schema_str = rt.block_on(async {
        let client = reqwest::Client::new();
        let resp = client.get(format!("{}/subjects/{}/versions/latest", url, subject))
            .send()
            .await?
