...
fn init_providers(log_file: &mut Option<std::fs::File>) {
    ...
    ONCE.call_once(|| {
            ...
            if let Some(exe_dir) = exe_path.parent() {
                ...
                if legacy_dll.exists() {
                    if let Some(dll_path_str) = legacy_dll.to_str() {
                         if let Some(ref mut f) = log_file {
                             writeln!(f, "Attempting to load legacy provider from path: {}", dll_path_str).ok();
                         }
                         let params = openssl::provider::Provider::try_load(None, dll_path_str, true);
                         match &params {
                             Ok(_) => {
                                 legacy_loaded = true;
                                 if let Some(ref mut f) = log_file {
                                     writeln!(f, "Success: Legacy provider loaded from absolute path.").ok();
                                 }
                                 // Prevent unloading
                                 if let Ok(p) = params { std::mem::forget(p); }
                             },
                             Err(e) => {
                                 if let Some(ref mut f) = log_file {
        ...
        if !legacy_loaded {
            ...
            if let Some(ref mut f) = log_file {
                 match &legacy {
                     Ok(_) => writeln!(f, "Legacy OpenSSL provider loaded successfully (by name).").ok(),
                     Err(e) => writeln!(f, "Failed to load Legacy OpenSSL provider (by name): {}", e).ok(),
                 };
            }
            if let Ok(p) = legacy {
                 std::mem::forget(p);
...
pub fn create_config(profile: &ClusterProfile) -> ClientConfig {
    ...
    if let (Some(username), Some(password), Some(mechanism)) = (&profile.sasl_username, &profile.sasl_password, &profile.mechanism) {
    ...
    */
    ...
    let ca_configured = config.get("ssl.ca.location").is_some() || config.get("ssl.ca.pem").is_some();
