...
pub fn convert_avro_value(
    value: &Value,
    schema: Option<&Schema>,
    resolved_schemas: &HashMap<Name, &Schema>,
    ...
    match value {
        ...
        // Complex Types
        Value::Bytes(b) => Ok(json!(format!("0x{}", hex::encode(b)))),
        Value::Fixed(_, b) => Ok(json!(format!("0x{}", hex::encode(b)))),
        Value::Enum(_index, symbol) => Ok(serde_json::Value::String(symbol.clone())),
        Value::Union(index, v) => {
        ...
        Value::Array(items) => {
            ...
            let vec: Result<Vec<_>, _> = items
                .iter()
                .map(|i| convert_avro_value(i, item_schema, resolved_schemas))
                .collect();
        ...
        Value::Map(items) => {
            ...
            for (k, v) in items {
                map.insert(
                    k.clone(),
                    convert_avro_value(v, value_schema, resolved_schemas)?,
                );
            }
        ...
        Value::Record(fields) => {
            ...
            for (k, v) in fields {
                ...
                map.insert(
                    k.clone(),
                    convert_avro_value(v, field_schema, resolved_schemas)?,
                );
            }
            Ok(serde_json::Value::Object(map))
        }
        ...
        Value::Decimal(d) => {
            ...
            if scale > 0 {
                ...
                let plain_digits = if is_negative {
                    &string_val[1..]
                } else {
                    &string_val
                };
...
mod tests {
    use super::*;
    use apache_avro::{
        schema::DecimalSchema, schema::Name, Days, Duration, Millis, Months, Schema,
    };
    ...
    fn test_primitives() {
        let map = HashMap::new();
        assert_eq!(
            convert_avro_value(&Value::Null, None, &map).unwrap(),
            serde_json::Value::Null
        );
        assert_eq!(
            convert_avro_value(&Value::Boolean(true), None, &map).unwrap(),
            json!(true)
        );
        assert_eq!(
            convert_avro_value(&Value::Int(123), None, &map).unwrap(),
            json!(123)
        );
        assert_eq!(
            convert_avro_value(&Value::Long(1234567890123), None, &map).unwrap(),
            json!(1234567890123u64)
        );
        assert_eq!(
            convert_avro_value(&Value::String("foo".to_string()), None, &map).unwrap(),
            json!("foo")
        );
    }
    ...
    fn test_bytes_fixed() {
        ...
        let bytes = vec![1, 2, 3];
        assert_eq!(
            convert_avro_value(&Value::Bytes(bytes.clone()), None, &map).unwrap(),
            json!("0x010203")
        );
        assert_eq!(
            convert_avro_value(&Value::Fixed(3, bytes), None, &map).unwrap(),
            json!("0x010203")
        );
    }
    ...
    fn test_enum() {
        let map = HashMap::new();
        assert_eq!(
            convert_avro_value(&Value::Enum(1, "SYMBOL".to_string()), None, &map).unwrap(),
            json!("SYMBOL")
        );
    }
    ...
    fn test_nested_with_decimal_in_record() {
        ...
        let decimal = Value::Decimal(apache_avro::Decimal::from(vec![0x30, 0x39]));
        let rec = Value::Record(vec![("d".to_string(), decimal)]);
    ...
    fn test_decimal_ref_resolution() {
        ...
        let record_val = Value::Record(vec![("value".to_string(), decimal_val)]);

        // Convert using the Ref schema
        let json =
            convert_avro_value(&record_val, Some(&ref_schema), &map).expect("Should convert ref");
