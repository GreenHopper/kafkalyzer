...
class ScriptTimelineWidget extends StatelessWidget {
  ...
  const ScriptTimelineWidget({super.key, required this.script, required this.runner});
        ...
        return LayoutBuilder(
          builder: (context, constraints) {
            ...
            for (int i = 0; i < stepCount; i++) {
              final status = runner.stepStatuses[script.steps[i].id] ?? StepStatus.pending;
              ...
              child: Stack(
                ...
                children: [
                  ...
                  Positioned(
                    ...
                    right: stepWidth / 2,
                    top: 23,
                    child: Container(height: 2, color: Colors.grey.withValues(alpha: 0.2)),
                  ),
                  ...
                  Row(
                    ...
                    children: script.steps.asMap().entries.map((entry) {
                      ...
                      final status = runner.stepStatuses[step.id] ?? StepStatus.pending;
                        ...
                        child: Column(
                          ...
                          children: [
                            MouseRegion(
                              ...
                              child: Tooltip(
                                message: "${step.name}: ${status.name.toUpperCase()}",
                                child: _buildStepIndicator(context, status, index + 1),
                              ),
                            ),
                          ],
                        ),
  ...
  Widget _buildStepIndicator(BuildContext context, StepStatus status, int index) {
    Widget child;
    ...
    switch (status) {
      case StepStatus.pending:
        child = Container(
          ...
          child: Text(
            "$index",
            style: TextStyle(color: Colors.grey.withValues(alpha: 0.7), fontWeight: FontWeight.bold, fontSize: 10),
          ),
        );
        break;
    ...
    if (status == StepStatus.pending) {
      return Container(width: 30, height: 30, alignment: Alignment.center, child: child);
    }
