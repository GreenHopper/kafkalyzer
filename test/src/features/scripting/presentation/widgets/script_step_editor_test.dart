...
class MockKafkaMetadataService extends KafkaMetadataService {
  @override
  Future<bool> validateConnection({required ClusterProfile profile}) async => true;
...
class MockClusterListController extends ClusterListController {
  MockClusterListController() : super(); // Adjust constructor if needed based on actual class
  @override
  List<ClusterProfile> get clusters => [const ClusterProfile(name: "Cluster A", bootstrapServers: "localhost:9092")];
}
...
class MockTopicController extends TopicController {
  ...
  List<TopicMetadata>? getTopics(ClusterProfile cluster) => [
    const TopicMetadata(name: "topic-a", partitionCount: 1, replicationFactor: 1),
    TopicMetadata(name: "long-topic-${'a' * 300}", partitionCount: 1, replicationFactor: 1),
  ];
  @override
...
void main() {
  setUpAll(() async {
    ...
    if (!GetIt.I.isRegistered<ClusterListController>()) {
      GetIt.I.registerSingleton<ClusterListController>(MockClusterListController());
    }
    if (!GetIt.I.isRegistered<KafkaMetadataService>()) {
      GetIt.I.registerSingleton<KafkaMetadataService>(MockKafkaMetadataService());
    }
  ...
  testWidgets('ScriptStepEditor handles long topic names without overflow', (WidgetTester tester) async {
    final longTopicName = "long-topic-${'a' * 300}";
    final step = ScriptStep(id: '1', name: 'Test Step', clusterName: 'Cluster A', topicNames: [longTopicName]);
  ...
  testWidgets('ScriptStepEditor shows chips for selected topics and allows removal', (WidgetTester tester) async {
    final step = ScriptStep(id: '1', name: 'Test Step', clusterName: 'Cluster A', topicNames: ['topic-a', 'topic-b']);
    ...
    // Remove 'topic-a'
    await tester.tap(find.descendant(of: find.widgetWithText(Chip, 'topic-a'), matching: find.byIcon(Icons.cancel)));
    await tester.pumpAndSettle();
